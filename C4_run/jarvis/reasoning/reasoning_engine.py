...
C4_SYSTEM_PROMPT = """You are C4 — A highly advanced Tactical AI.
...
- MULTI-LINGUAL: C4 is fluent in English, Hindi, and Marathi. Respond in the same language the user uses. If the user speaks a mix (e.g., Hinglish), respond in clear English or the dominant language.
"""
...
class ReasoningEngine:
    ...
    def handle_intent(self, intent: Intent, context: ConversationContext) -> ReasoningResponse:
        ...
        # Removed aggressive voice acknowledgement to minimize repetitiveness.
...
Supported types: open_app, open_url, create_file, play_media, run_command."""
    ...
    def _handle_write_code_active_window(self, intent: Intent, context: ConversationContext) -> ReasoningResponse:
        ...
        if voice:
            import threading
            threading.Thread(
                target=lambda: voice.speak_thinking("Writing code directly into the active window, sir."),
                daemon=True
            ).start()

        # Detect web requests and inject the design system
        is_web = any(w in request.lower() for w in [
            "html", "website", "webpage", "web page", "portfolio", "landing page",
            "site", "css", "frontend", "front end"
        ])

        from jarvis.reasoning.prompts import C4_WEB_DESIGN_SYSTEM
        design_block = (
            f"\n\nUSE THIS EXACT CSS DESIGN SYSTEM (embed it inside a <style> block in the HTML):\n{C4_WEB_DESIGN_SYSTEM}"
            if is_web else ""
        )

        prompt = f"""You are C4 Coder. Write the COMPLETE, FINISHED code for this request.

REQUEST: {request}
{design_block}

ABSOLUTE RULES — violation means failure:
1. Output ONLY the final, working code. Zero placeholders. Zero template comments like "<!-- content here -->".
2. Every section must have REAL content — real headings, real paragraph text, real nav links, real skills listed.
3. For HTML: use the CSS variables from the design system (--c-accent, --c-bg, --c-surface, etc). Add fadeUp animations. Use Outfit font for headings.
4. Do NOT output ```html or any markdown fences whatsoever.
5. Do NOT output explanations, intros, or sign-offs.
6. The output must be the FULL file — not a snippet, not a skeleton.
"""

        # Use dedicated webgen role (codeqwen 12k ctx) for web, coder role for everything else
        role = "webgen" if is_web else "coder"
        code_text = self._run_role(role, prompt, context=context)

        if not code_text or not code_text.strip():
            return ReasoningResponse(spoken_response="I could not generate any code for that request, sir.")

        # Strip any markdown fences the LLM adds despite being told not to
        ...
        # For HTML: truncate anything after the closing </html> tag
        # (models sometimes append explanation prose after the code)
        if is_web and "</html>" in code_text.lower():
            idx = code_text.lower().rfind("</html>")
            code_text = code_text[: idx + len("</html>")].strip()

        self._last_generated_code = code_text

        if self.executor:
            self.executor.execute_step({"type": "keyboard_type", "text": code_text, "interval": 0.01})
            return ReasoningResponse(spoken_response="Done, sir. Full website written into your active window.")
        else:
