...
class VADInterruptor:
    """Continually monitors mic energy and publishes an interrupt event if a loud spike occurs."""
    def __init__(self, threshold: int = 1500):
        self.threshold = threshold
        self.running = False
        self.thread = threading.Thread(target=self._run, daemon=True, name="VADInterruptor")
    ...
    def start(self):
        ...
        self.running = True
        self.thread.start()
        logger.info(f"[VAD] Started background activity interrupt monitor (Threshold: {self.threshold})")
    ...
    def _run(self):
        try:
            ...
            logger.debug(f"[VAD] Monitor failed to start (PyAudio not accessible in background): {e}")
...
class SpeechRecognitionInput(VoiceInput):
    ...
    def __init__(
        ...
        # Start VAD background interrupter
        self.vad = VADInterruptor(threshold=self.energy_threshold + 500)
        self.vad.start()
